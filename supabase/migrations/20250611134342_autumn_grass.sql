/*
  # Database Migration to UUID Primary Keys

  1. New Schema Changes
    - Convert all primary keys from BIGINT/SERIAL to UUID
    - Update all foreign key references
    - Ensure all new UUIDs are version 4 (random)
    - Maintain existing data by converting IDs
  
  2. Migration Strategy
    - Create temporary tables with UUID structure
...
SELECT 
  ...
  log_message, 
  created_at, 
  convert_id_to_uuid(new_user_id)
FROM security_logs;
...
-- 17. Migrate sessions and customers tables
...
DROP TABLE IF EXISTS storage_access_logs;
...
ALTER INDEX idx_notifications_new_user_id RENAME TO idx_notifications_user_id;
...
ALTER INDEX idx_user_profiles_new_user_id RENAME TO idx_user_profiles_user_id;
...
-- Update RLS policies
-- This would be a good place to update RLS policies to use UUIDs
-- For each table with RLS enabled, recreate the policies
...
-- This is not necessary for UUID primary keys