...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'clients_new_email_key') THEN
    CREATE UNIQUE INDEX clients_new_email_key ON clients_new(email);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_clients_new_email') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_bookings_new_client_id') THEN
    CREATE INDEX idx_bookings_new_client_id ON bookings_new(client_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_bookings_new_yacht_id') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_payments_new_booking_id') THEN
...
-- Check if notifications table exists and has the necessary columns
DO $$
DECLARE
...
BEGIN
  ...
  SELECT 
    EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'notifications' AND column_name = 'recipient_id'),
    EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'notifications' AND column_name = 'booking_id'),
    EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'notifications' AND column_name = 'user_id')
  INTO has_recipient_id, has_booking_id, has_user_id;

  -- Insert data based on available columns
  IF has_recipient_id AND has_booking_id THEN
    -- Both recipient_id and booking_id exist
  ...
  ELSIF has_user_id THEN
    -- Only user_id exists (map it to recipient_id)
    INSERT INTO notifications_new (
      id, 
      message, 
      is_read, 
    ...
    FROM notifications;
  ELSE
    -- Neither recipient_id nor booking_id exists
    INSERT INTO notifications_new (
      id, 
      message, 
      is_read, 
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_notifications_new_recipient_id') THEN
    CREATE INDEX idx_notifications_new_recipient_id ON notifications_new(recipient_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_notifications_new_booking_id') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_webhook_logs_new_webhook_id') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_subscription_events_new_user_id') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_file_metadata_new_user_id') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_storage_files_new_user_id') THEN
...
-- Check if storage_access_logs table exists
DO $$
DECLARE
  table_exists boolean;
BEGIN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_storage_access_logs_new_user_id') THEN
    CREATE INDEX idx_storage_access_logs_new_user_id ON storage_access_logs_new(user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_storage_access_logs_new_file_id') THEN
...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'clients_new_email_key') THEN
    ALTER INDEX clients_new_email_key RENAME TO clients_email_key;
  END IF;
  
  IF EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_clients_new_email') THEN
    ALTER INDEX idx_clients_new_email RENAME TO idx_clients_email;
  END IF;
  
  IF EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_bookings_new_client_id') THEN
    ALTER INDEX idx_bookings_new_client_id RENAME TO idx_bookings_client_id;
  END IF;
  
  IF EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_bookings_new_yacht_id') THEN
    ALTER INDEX idx_bookings_new_yacht_id RENAME TO idx_bookings_yacht_id;
  END IF;
  
  IF EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_payments_new_booking_id') THEN
    ALTER INDEX idx_payments_new_booking_id RENAME TO idx_payments_booking_id;
  END IF;
  
  IF EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_notifications_new_recipient_id') THEN
    ALTER INDEX idx_notifications_new_recipient_id RENAME TO idx_notifications_recipient_id;
  END IF;
  
  IF EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_notifications_new_booking_id') THEN
    ALTER INDEX idx_notifications_new_booking_id RENAME TO idx_notifications_booking_id;
  END IF;
  
  IF EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_webhook_logs_new_webhook_id') THEN
    ALTER INDEX idx_webhook_logs_new_webhook_id RENAME TO idx_webhook_logs_webhook_id;
  END IF;
  
  IF EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_subscription_events_new_user_id') THEN
    ALTER INDEX idx_subscription_events_new_user_id RENAME TO idx_subscription_events_user_id;
  END IF;
  
  IF EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_file_metadata_new_user_id') THEN
    ALTER INDEX idx_file_metadata_new_user_id RENAME TO idx_file_metadata_user_id;
  END IF;
  
  IF EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_storage_files_new_user_id') THEN
    ALTER INDEX idx_storage_files_new_user_id RENAME TO idx_storage_files_user_id;
  END IF;
  
  IF EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_storage_access_logs_new_user_id') THEN
    ALTER INDEX idx_storage_access_logs_new_user_id RENAME TO idx_storage_access_logs_user_id;
  END IF;
  
  IF EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_storage_access_logs_new_file_id') THEN
